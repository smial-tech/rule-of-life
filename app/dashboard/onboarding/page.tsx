...
import {Rocket01Icon, Users, Folder01Icon, Plug01Icon, CircleCheck, LoaderCircle} from '@hugeicons/core-free-icons';
  ...
  if (isCompleted) {
    return (
      <div className="flex size-7 items-center justify-center">
        <HugeiconsIcon icon={CircleCheck} strokeWidth={2} aria-hidden="true" className="size-7 text-emerald-500" />
            ...
            <div className="min-w-0 flex-1">
              ...
              {isActive && (
                <Button className="mt-3" onClick={onComplete} size="sm">
                  <HugeiconsIcon icon={StepIcon} strokeWidth={2} aria-hidden="true"
                    className="-ml-0.5 size-4 shrink-0"/>
                  {step.actionLabel}
                </Button>
              )}
            </div>
            <HugeiconsIcon icon={StepIcon} strokeWidth={2} aria-hidden="true" className={cn(
...
export default function OnboardingPage() {
  ...
  if (isPending) {
    return <div className="flex min-h-dvh items-center justify-center bg-background p-4">
      <HugeiconsIcon icon={LoaderCircle} strokeWidth={2} className="size-10 animate-spin text-primary" />
    </div>
  }
